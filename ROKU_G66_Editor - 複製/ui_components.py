...
class DrillingPlot(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        gs = self.figure.add_gridspec(1, 2, width_ratios=[1.3, 2])
        self.ax_geo = self.figure.add_subplot(gs[0])
        self.ax_cycle = self.figure.add_subplot(gs[1])
        self.figure.subplots_adjust(left=0.08, right=0.95, top=0.9, bottom=0.1, wspace=0.2)
    ...
    def draw_geometry(self, r_val, z_val, visual_params, tool_z_override=None, keep_limits=False):
        ...
        # Material Body
        self.ax_geo.fill_between([-display_width, display_width], material_bottom, material_top, 
                                 color='#D3D3D3', alpha=0.5, label='素材', zorder=1)
    ...
    def draw_cycle(self, r_val, z_val, ijk_list, visual_params, highlight_peck_idx=None, 
        ...
        # G66: Tool sit at S (Approach); Others: sit at R
        if not is_g83:
            start_nc_z = visual_params.get('S', r_val)
        else:
            start_nc_z = r_val
            
        current_z_visual = start_nc_z + shift
        ...
        if ijk_list:
            for idx, params in enumerate(ijk_list):
                ...
                if not is_g83:
                    # NC Coordinates
                    target_nc_z = val_i # Absolute target depth
                    stage_peck_q = params.get('J', 0.0)
                    if stage_peck_q <= 0: stage_peck_q = 1.0
                    approach_nc_z = visual_params.get('S', 0.0)
                    
                    # 階段初始化：首刀流程 直接由 R 點開始進給
                    if idx == 0:
                        last_drilled_nc_z = r_val
                        sim_current_nc_z = r_val
                    else:
                        sim_current_nc_z = last_drilled_nc_z 
                        
                    is_drilling_down = target_nc_z < sim_current_nc_z
                    
                    # 用於追蹤本階段的起始點
                    stage_start_nc_z = sim_current_nc_z
                    while True:
                        if is_drilling_down:
                        ...
                        if idx == 0 and last_drilled_nc_z == r_val:
                            clearance_nc_z = r_val
                        else:
                            if is_drilling_down:
                                clearance_nc_z = last_drilled_nc_z + gap_d
                                if clearance_nc_z > approach_nc_z: clearance_nc_z = approach_nc_z
                            else:
                                clearance_nc_z = last_drilled_nc_z - gap_d
                        ...
                        self.feed_z.extend([clearance_nc_z + shift, next_nc_z + shift, None])
                        self.feed_ijk_idx_map.extend([idx, idx, idx])
                        current_x += 1.0
                        
                        # 3. 退刀 (Retract/Chip breaking)
                        self.rapid_x.extend([current_x, current_x + 0.2, None])
        ...
        if keep_limits and prev_xlim and prev_ylim:
            ...
            y_min = min(program_z_visual, material_bottom) - 1
            y_max = max(program_r_visual, material_top) + 1
            
            if y_min > min(z0_visual, material_bottom) - 1: y_min = min(z0_visual, material_bottom) - 1
            if y_max < max(z0_visual, material_top) + 1: y_max = max(z0_visual, material_top) + 1
...
class ParamTable(QTableWidget):
    ...
    def load_data(self, ijk_list):
        ...
        for r, row_data in enumerate(ijk_list):
            self.setItem(r, 0, QTableWidgetItem(str(row_data.get('I', 0.0))))
            self.setItem(r, 1, QTableWidgetItem(str(row_data.get('J', 0.0))))
            self.setItem(r, 2, QTableWidgetItem(str(row_data.get('K', 0.0))))
