...
class TestRokuParser(unittest.TestCase):
    ...
    def test_parse_logic(self):
        ...
        tool = data[0]
        # [M7 修復] tool_id 實際儲存為數字字串 (不含 'T' 前綴)
        self.assertEqual(tool['tool_id'], "10")
        ...
        # [C7 修復] _is_zero_set 會過濾全零組 (I0 J0 K0)，實際只有 2 組
        ...
        # Set 2: I-1.0 J0.2 K50. (原 Set 3，Set 2 全零已被過濾)
        self.assertEqual(dynamics[1]['I'], -1.0)
        self.assertEqual(dynamics[1]['J'], 0.2)
        self.assertEqual(dynamics[1]['K'], 50.0)
    ...
    def test_rebuild_logic(self):
        ...
        # [修正] :g 格式會移除尾端 .0 (如 -3.0 → -3, 20.0 → 20)
        # 全零組 (I0 J0 K0) 會被 _is_zero_set 過濾而不輸出
        expected_fragment = "G66 P9131 R-0.5 Z-3 S0.05 I-1.5 J0.1 K20"
